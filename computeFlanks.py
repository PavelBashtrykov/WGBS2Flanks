...
def main():
    ...
    for flank_size in range(1, 4):
        empty_flanks_dict = _make_flanks_dict(flank_size)
        flanks_stat = _compute_flanks_methylation(
            flank_size, empty_flanks_dict, infile, seq_depth
        )
...
def _compute_flanks_methylation(
    flank_size: int, flank_dict: OrderedDict, infile: str, depth_limit: int = 10
):
    ...
    """
    fh = open(infile, "r")
    seq_start = int(22 / 2 - len("CG") / 2 - flank_size)
    seq_end = int(22 / 2 + len("CG") / 2 + flank_size)
    line = fh.readline()
    while line:
